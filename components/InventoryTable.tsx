...
const InventoryTable: React.FC<InventoryTableProps> = ({ chemicals, onEdit, onDelete, onGetSafetyInfo, onOpenAdjustModal, onOpenUsageLog, onOpenBarcodeModal, onOpenSafetyDoc }) => {
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {sortedChemicals.length > 0 ? sortedChemicals.map(chemical => {
              ...
              return (
                <tr key={chemical.id} className={`transition-colors duration-200 hover:bg-slate-100 ${status.rowClass}`}>
