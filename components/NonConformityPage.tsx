...
interface NonConformityPageProps {
  ...
  onAddOrUpdate: (item: NonConformity | null) => void;
...
const NonConformityPage: React.FC<NonConformityPageProps> = (props) => {
  const { 
      nonConformities, onAddOrUpdate, onDelete, onExportToDoc, onExportToExcel, 
  ...
  const correctiveActionItems = useMemo(() => {
    return nonConformities.filter(nc => 
      nc.status !== 'closed' && nc.correctiveAction && nc.correctiveAction.trim() !== ''
  ...
  const preventiveActionItems = useMemo(() => {
    return nonConformities.filter(nc => 
      nc.status !== 'closed' && nc.preventiveAction && nc.preventiveAction.trim() !== ''
    ).sort((a,b) => a.date.localeCompare(b.date)); // Sort by date, oldest first
  }, [nonConformities]);
      ...
      {activeTab === 'list' && (
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50">
                <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Mã số SKPH</th>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
                ...
                <tr key={item.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-700">{item.ncId}</td>
        ...
        <div>
           <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-slate-500">
                    Danh sách các hành động khắc phục cần thực hiện cho các SKPH đang mở hoặc đang xử lý.
                </p>
                <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            {filteredCorrectiveActionItems.length > 0 ? filteredCorrectiveActionItems.map(item => {
                ...
                <div key={item.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow bg-white">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-end gap-2">
                        ...
                        <button onClick={() => onAddOrUpdate(item)} className="flex-shrink-0 inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-700 text-sm font-semibold rounded-md hover:bg-blue-200">
              ...
              <div className="text-center py-16 text-slate-500 bg-slate-50 rounded-lg">
                ...
                <h3 className="font-semibold">Đã hoàn thành!</h3>
                <p>Không có hành động khắc phục nào cần thực hiện trong khoảng thời gian đã chọn.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {activeTab === 'preventive_actions' && (
        <div>
          <p className="text-sm text-slate-500 mb-4">
            Danh sách các hành động phòng ngừa cần thực hiện cho các SKPH.
          </p>
          <div className="space-y-4">
            {preventiveActionItems.length > 0 ? preventiveActionItems.map(item => {
              const status = statusMap[item.status];
              return (
                <div key={item.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow bg-white">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-grow">
                      ...
                      <p className="mt-1 font-semibold text-slate-800">SKPH: {item.description}</p>
                      <p className="mt-2 text-sm text-slate-600">
                        <strong className="text-slate-800">Hành động phòng ngừa:</strong> {item.preventiveAction}
                      </p>
                    </div>
                    <button onClick={() => onAddOrUpdate(item)} className="flex-shrink-0 inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-700 text-sm font-semibold rounded-md hover:bg-blue-200">
                        <EditIcon className="w-4 h-4" /> Cập nhật
              ...
              <div className="text-center py-16 text-slate-500 bg-slate-50 rounded-lg">
                ...
                <h3 className="font-semibold">Tuyệt vời!</h3>
                <p>Không có hành động phòng ngừa nào cần thực hiện.</p>
              </div>
            )}
          </div>
        </div>
